...
export function computeGradScore(input: GradScoreInput): GradScore {
  ...
  const fields = [
    str("full_name"), str("university"), str("degree_level"), str("degree_subject"),
    str("country_of_origin"), str("visa_type"), str("visa_expiry_date"), str("target_industry"),
    hasRoles ? "x" : "", str("experience_level"), str("linkedin_url"),
  ];
  ...
  const tips: string[] = [];
  if (completeness < 80) tips.push("Complete your profile to sharpen every score and your matches.");
