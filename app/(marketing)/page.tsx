            ...
            <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {testimonials.slice(0, 6).map((t) => (
                <figure key={t.id} className="card flex flex-col p-6">
                  ...
                  <blockquote className="mt-3 flex-1 text-sm text-slate-700">&ldquo;{t.content}&rdquo;</blockquote>
                  <figcaption className="mt-4 flex items-center gap-3">
                    {t.avatar_url ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={t.avatar_url} alt="" className="size-9 rounded-full bg-slate-200 object-cover" />
                    ) : (
